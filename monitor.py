...
def check_tss(
    ...
    if not TSS_DIR.exists():
        return (
            "UNKNOWN",
            None,
            "Modern_TSS_Checker directory missing",
        )
    ...
    try:
        ...
        return (
            "UNKNOWN",
            None,
            f"TSS checker launch failed: {exc}",
        )
...
def render_alert(
    ...
    subject = (
        "🚨 Apple Signing Status Changed — "
        f"{result['name']} — "
        f"iOS {result['version']} "
        f"({result['buildid']})"
    )

    previous_ipsw = previous_record.get(
        "ipsw_status",
        "UNKNOWN",
    )

    previous_tss = previous_record.get(
        "tss_status",
        "UNKNOWN",
    )
...
def main() -> int:
    ...
    for target in targets:
        ...
        firmwares = fetch_ipsw_firmwares(
            device
        )
    ...
    print(
        f"Completed: "
        f"{len(all_results)} builds checked, "
        f"{len(alerts)} email(s) sent."
    )
